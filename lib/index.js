...
module.exports = constructorOptions => {
  ...
  // 30 seconds
  const defaultTTL = utils.isNumberOrString(constructorOptions.ttl)
    ? utils.ms(constructorOptions.ttl)
    : utils.ms('30s')
  // prefix
  const defaultTimeout = utils.isNumberOrString(constructorOptions.timeout)
    ? utils.ms(constructorOptions.timeout)
    : utils.ms('30s')
  // onError
  const defaultOnError = constructorOptions.onError || utils._onError
  ...
  const defaultMinimumTimeoutPollInterval = utils.ms(constructorOptions.minimumPollInterval || 100)
  // function to create a time out error
  const defaultCreateTimeoutError = constructorOptions.createTimeoutError || utils._createTimeoutError
  ...
  return decoratorOptions => {
    ...
    return fn => {
      ...
      return Promise.coroutine(function * () {
        ...
        // bypass the cache
        if (disabled) return yield utils.executeFunction(fn, args)
